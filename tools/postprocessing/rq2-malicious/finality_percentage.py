...
def analyze_finality_per_simulation(df, threshold):
    ...
    Finality is defined as reaching the confidence threshold and sustaining it for the
    remainder of that specific simulation.
    """
    ...
    for tx_id in transactions:
        ...
        for sim_id in sim_ids:
            ...
            # Find all times where confidence meets the threshold
            candidate_times = sim_tx_df[sim_tx_df['conf'] >= threshold]['Time']

            # Iterate through candidate times to find if any lead to sustained confidence
            finality_achieved_in_sim = False
            for time_val in candidate_times:
                subsequent_df = sim_tx_df[sim_tx_df['Time'] >= time_val]
                if (subsequent_df['conf'] >= threshold).all():
                    # Sustained condition met for this simulation
                    finality_achieved_in_sim = True
                    break  # Move to the next simulation
...
def process_directory(dir_name):
    ...
    # Calculate confidence per simulation, transaction, and time
    net_conf = belief_df.groupby(["SimID", "Transaction", "Time"])["Believes"].mean().reset_index(name="conf")

    # --- Finality per Simulation Analysis ---
    finality_sim_results = analyze_finality_per_simulation(net_conf, FIXED_BELIEF_THRESHOLD)
    logging.info(f"Directory '{dir_name}' -> Per-simulation finality calculated.")

    # Calculate average confidence across all simulations for plotting and overall analysis
    ...
    # --- Confidence Sustain Analysis (based on average) ---
    ...
    # --- Add the per-simulation finality results ---
    ...
    # Ensure new columns are integers where possible
